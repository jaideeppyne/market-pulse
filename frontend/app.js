(() => {
  ...
  // Portfolio / Journal button wiring
  ...
  document.getElementById("exportJournalBtn")?.addEventListener("click", () => {
    const rows = (journalData || []).map(j => ({
      symbol: j.symbol, action: j.action, price: j.price, qty: j.qty,
      pnl: j.outcome_pnl, score: j.score_at_time, notes: j.notes
    }));
    exportCSV(rows, "market_pulse_journal.csv");
  });
  document.getElementById("logPaperBuyBtn")?.addEventListener("click", async () => {
    ...
    const qty = parseFloat(document.getElementById("portQty")?.value || "100");
    const sl = parseFloat(document.getElementById("portSL")?.value || "") || null;
    const tgt = parseFloat(document.getElementById("portTarget")?.value || "") || null;
    const notes = document.getElementById("portNotes")?.value || null;
    try {
      await addToPortfolio(sym, {qty, sl, target: tgt, notes});
      // clear form
      ["portSymbol","portQty","portSL","portTarget","portNotes"].forEach(id => { const el=document.getElementById(id); if(el) el.value = (id==="portQty"?"100":""); });
      await renderPortfolio(true);
    } catch(e){ alert("Failed to log paper buy: " + e); }
  });
  ...
  function renderHot(data) {
    ...
    hotBody.innerHTML = rows
      .map((r, idx) => {
        ...
        return `<tr class="${sel} ${flash}${whale ? " row-whale" : ""}" data-symbol="${attrEsc(r.symbol)}">
          <td><span class="rank-num">${idx + 1}</span> <strong class="clickable" data-act="select">${r.symbol}</strong>${ext}${discBadge}${fullBadge}${whaleClickable}${investorLine}<br><span class="sym-name">${escapeHtml(m.name || "")}</span> <button class="tiny-watch" data-watch="${attrEsc(r.symbol)}" title="Add/remove from My List">${watched}</button>${paperBtnHtml}</td>
    ...
    hotBody.querySelectorAll("tr").forEach((tr) => {
      tr.addEventListener("click", (ev) => {
        if (ev.target.closest(".factor-pill") || ev.target.closest(".tiny-watch") || ev.target.closest("[data-whale-sym]") || ev.target.closest("[data-act]") || ev.target.closest("[data-paper]")) return;
    ...
    // One-click paper trade from hot (ties directly to analyze data + thesis)
    hotBody.querySelectorAll("[data-paper]").forEach((btn) => {
      btn.addEventListener("click", (ev) => {
        ev.stopPropagation();
        const sym = btn.dataset.paper;
        const row = findRow(sym, data);
        addToPortfolio(sym, {qty: 100}).catch(() => {});
        // also flash portfolio tab hint
        setTimeout(() => { const t = document.querySelector('.tab[data-tab="portfolio"]'); if (t) t.style.outline="1px solid #22c55e"; setTimeout(()=> t && (t.style.outline=""), 900); }, 120);
      });
    });
  ...
  function renderDetail(row) {
    ...
    detailEl.innerHTML = `<div class="detail">
      ...
      <div style="margin-top:0.4rem"><button class="tiny" data-act="whale">🐳 Whale details</button> <button class="tiny" data-act="thesis">Copy full thesis</button> <button class="tiny" data-act="watch">${isWatched ? "✓ Watched" : "★ Watch"}</button> <button class="tiny" data-act="paper">📁 Add to Portfolio</button></div>
    </div>`;
    ...
    extraBar?.querySelector('[data-act="paper"]')?.addEventListener("click", () => {
      addToPortfolio(row.symbol, {qty: 100, notes: (buildPositivesAndNegatives(row).positives || []).slice(0,2).join(" · ")});
    });
    ...
    thesisBlock?.querySelector('[data-act="paper"]')?.addEventListener("click", () => {
      addToPortfolio(row.symbol, {qty: 100});
    });
    ...
    tbody.querySelectorAll("tr").forEach(tr => {
      tr.addEventListener("click", (e) => {
        ...
        if (e.target.dataset.act === "paper") {
          addToPortfolio(sym, {qty: 100}); return;
        }
        if (e.target.dataset.act === "analyze" || !e.target.dataset.act) {
          document.querySelector('.tab[data-tab="hot"]')?.click();
  ...
  function applyUpdate(data) {
    ...
    if (activeTab === "watch") renderWatch(data);
    if (activeTab === "portfolio") renderPortfolio();  // live Buy/Qual/PnL refresh from hot updates (state.symbols)
  }
