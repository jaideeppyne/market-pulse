(() => {
  ...
  function renderStats(data) {
    ...
    statsBar.innerHTML = `
      ...
      <div class="stat-card clickable" data-stat="edge" title="Click: load Backtest Edge / Historical Performance (rich 1d/3d/7d/14d hit rates by score bucket + conf breakdowns + factor edge + max DD + regime from /api/edge)"><div class="label">📊 Backtest Edge</div><div class="value" style="font-size:0.65rem">Stats</div></div>
      ${lastRegime ? `<div class="stat-card" title="${attrEsc(lastRegime.note || '')}"><div class="label">Regime</div><div class="value" style="font-size:0.68rem">${lastRegime.high_vol ? '⚠️' : '🌍'} VIX ${lastRegime.vix}</div></div>` : ''}
    `;
    bindHelp(statsBar);
